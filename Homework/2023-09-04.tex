...
%
\subsubsection*{EXERCÍCIO x3.5. \\
Demonstre o Teorema $\theta$3.10. Cuidado: terminando uma, não faz sentido a outra ocupar
um espaço parecido no teu texto!}
%
(ZA-CanR) $(\forall a, b, c)[ a + c = b + c \implies a = b ]$
%
\begin{table}[H]
  \begin{dem}
    \linha{sejam $a, b, c$ inteiros}
    \linha{suponha $a + c = b + c$}
    \linha{calculamos:}
  \end{dem}
  \begin{alvodados}
    \cmnt{$a$}{$a = b$}
...
\begin{align*}
  a &= a + 0 \tag*{[(ZA-IdR) a]}\\
  & = a + (c - c) \tag*{[(ZA-InvR) c]}\\
  &= (a + c) - c \tag*{[(ZA-Ass) a c -c]}\\
  &= (b + c) - c \tag*{[1]}\\
  &= b + (c - c) \tag*{[(ZA-Ass) b c -c]}\\
  &= b + 0 \tag*{[(ZA-InvR) c]}\\
  &= b \tag*{[(ZA-IdR) b]}
\end{align*}
%
(ZA-CanL) $(\forall a, b, c)[ c + a = c + b \implies a = b ]$
%
\begin{table}[H]
  \begin{dem}
    \linha{sejam $a, b, c$ inteiros}
    \linha{suponha $c + a = c + b$}
    \linha{logo $a + c = c + b$, por [(ZA-Com) c a]}
    \linha{logo $a + c = b + c$, por [(ZA-Com) c b]}
    \linha{logo $a = b$, por [(ZA-CanR) a b c]}
  \end{dem}
  \begin{alvodados}
    \cmnt{$a$}{$a = b$}
    \cmnt{$b$}{}
...
\end{table}
%
\subsubsection*{EXERCÍCIO x3.6.\\
Enuncie e refute a proposição correpsondente para a multiplicação.}
%
(ZM-CanR) $(\forall a, b, c)[ a \cdot c = b \cdot c \implies a = b ]$
%
Por contra-exemplo: suponha $a=1$, $b=2$ e $c=0$. Nesse caso, $a \cdot c = 0 = b \cdot c \centernot \implies a = b$.
%
\subsubsection*{EXERCÍCIO x3.7.\\
Demonstre o Unicidade da ( $\cdot$ )-identidade $\theta$3.12}
%
$(\exists i)(\forall a)[a \cdot i = a~\&~i \cdot a = a]~\&~(\forall i, j)(\forall a)[(i \cdot a = a~\&~a \cdot i = a)~\&~(j \cdot a = a~\&~a \cdot j = a) \implies i = j]$
%
\begin{table}[H]
  \begin{dem}
    \linha{split}
  \end{dem}
  \begin{alvodados}
    \cmnt{}{$(\exists i)(\forall a)[a \cdot i = a~\&~i \cdot a = a]~\&~(\forall i, j)(\forall a)[(i \cdot a = a~\&~a \cdot i = a)~\&~(j \cdot a = a~\&~a \cdot j = a) \implies i = j]$}
  \end{alvodados}
\end{table}
%
\begin{table}[H]
